...
const startExam = async (user_id, exam_id) => {
  ...
  // Enforce attempts count limit for starting a new attempt
  const completedCount = await ExamModel.countCompletedAttempts(user_id, exam_id);
  if (completedCount >= exam.max_attempts) {
    throw new Error(`Maximum attempts limit reached (${exam.max_attempts} attempt(s) allowed)`);
  }
