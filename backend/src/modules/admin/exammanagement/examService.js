...
const createExam = async (examData) => {
  ...
  if (Array.isArray(examData.recommended_courses)) {
    await ExamModel.setRecommendedCourses(result.insertId, examData.recommended_courses);
  }
...
// Get All Exams
const getAllExams = async (search, is_published, is_independent) => {
  const exams = await ExamModel.getAllExams(search, is_published, is_independent);
...
const updateExam = async (exam_id, examData) => {
  ...
  if (Array.isArray(examData.recommended_courses)) {
    await ExamModel.setRecommendedCourses(exam_id, examData.recommended_courses);
  }
