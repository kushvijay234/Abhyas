...
const createCourse = async (courseData) => {
  ...
  const [result] = await pool.execute(
    `INSERT INTO courses (title, description, category_id, thumbnail, status, duration)
     VALUES (?, ?, ?, ?, ?, ?)`,
    [title, description, category_id, thumbnail, status, duration]
  );
...
const updateCourse = async (course_id, courseData) => {
  ...
  const [result] = await pool.execute(
    `UPDATE courses
     ...
     WHERE course_id = ?`,
    [title, description, thumbnail, status, category_id, duration, course_id]
  );
...
const assignCourseCategory = async (course_id, category_id) => {
  const [result] = await pool.execute(
    `UPDATE courses SET category_id = ? WHERE course_id = ?`,
